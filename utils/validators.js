...
exports.registerValidators = [
    ...
    body('name', 'Имя должно быть минимум 2 символа').isLength({ min: 2, max: 150 }).trim(),
    // body('name', 'Имя должно быть записано только киррилицей').isAlpha('ru-RU'),
    body('name', 'Имя должно содержать от 1 до 3 слов').custom((value, { req }) => {
        const words = value.split(' ');
        const minWords = 1;
        const maxWords = 3;

        if (words.length < minWords || words.length > maxWords) {
            throw new Error(`Имя должно содержать от ${minWords} до ${maxWords} слов`);
        }
